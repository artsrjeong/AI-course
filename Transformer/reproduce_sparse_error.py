...
class PositionalEncoding(tf.keras.layers.Layer):
  ...
  def call(self, inputs):
    # Try to catch the sparsity here
    print(f"DEBUG: inputs type: {type(inputs)}")
    if hasattr(inputs, 'sparse'):
        print(f"DEBUG: inputs.sparse: {inputs.sparse}")
    
    # If we detect it's symbolic and potentially sparse, we might need a workaround
